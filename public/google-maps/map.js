...
const increaseInterval = () => {
  document.querySelector("input[name=interval]").value =
    Number(document.querySelector("input[name=interval]").value) + 100;
};
...
const initGoogleMap = (domId, coord, zoomLevel) => {
  ...
  window.sample_data = {
    ...
    properties: {
      EMD_CD: "27260104",
      EMD_ENG_NM: "Suseong-dong 2(i)-ga",
      EMD_KOR_NM: "\uc218\uc131\ub3d92\uac00",
    },
    geometry: {
      type: "Polygon",
      coordinates: [
        [
          [128.6132732784135, 35.85113899945861],
          [128.61410788409898, 35.859773498500566],
          [128.61763129693347, 35.85959554439207],
          [128.61676622240017, 35.850922109231455],
          [128.6132732784135, 35.85113899945861],
        ],
      ],
    },
...
function addPolygonToMap() {
  if (data.geojson) {
    ...
    for (let coord of data.coords) {
      p.push([coord.lng, coord.lat]);
    }
    p.push([data.coords[0].lng, data.coords[0].lat]);
    coords2.push(p);
